class FoodItem:
    def __init__(self, name="Water", fat=0.0, carbs=0.0, protein=0.0): #note that "Water" defaults macros to zero
        ...
        self.fat = fat
        self.carbs = carbs
        self.protein = protein #add name-protien
...
if __name__ == "__main__":
    ...
    if item_name.lower() == 'water':
        ...
        food_item.print_info()
        print(f'Number of calories for {1.0:.2f} serving(s): {food_item.get_calories(1.0):.2f}') #add decimal restrictions
        print(f'Number of calories for {num_servings:.2f} serving(s): {food_item.get_calories(num_servings):.2f}') #add decimal restrictions